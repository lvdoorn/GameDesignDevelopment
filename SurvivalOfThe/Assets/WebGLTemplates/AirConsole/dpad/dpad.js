...
function DPad(el, opts) {
  ...
  me.elements = {};
  me.elements[DPad.UP] = el.getElementsByClassName("arrowU")[0];
  me.elements[DPad.DOWN] = el.getElementsByClassName("arrowD")[0];
  me.elements[DPad.LEFT] = el.getElementsByClassName("arrowL")[0];
  me.elements[DPad.RIGHT] = el.getElementsByClassName("arrowR")[0];
  me.resetState();
...
DPad.prototype.setState = function(direction, active) {
  ...
  if (me.state[direction] != active) {
    ...
    if (active) {
      me.elements[direction].className += " active";
    } else {
      me.elements[direction].className =
          me.elements[direction].className.replace(/ active/g, "");
    }
    return true;
  }
  return false;
